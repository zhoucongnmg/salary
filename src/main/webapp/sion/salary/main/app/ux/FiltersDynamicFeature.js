 ...
 *     grid.filters
 */
Ext.define('Ext.ux.grid.FiltersDynamicFeature', {
    extend: 'Ext.ux.grid.FiltersFeature',
    alias: 'feature.dynamicfilters',
    uses: [
        ...
        'Ext.ux.grid.filter.TreepickerFilter'
    ],
    ...
    onBeforeLoad : function (store, options) {
    	...
    	proxy.doRequest = function(operation, callback, scope){
    ...
    buildQuery : function (filters) {
        var p = {}, i, f, root, dataPrefix, key, value, tmp,
            len = filters.length, condition;
        ...
        if (!this.encode){
            ...
            for(f in filters){
            	filter = filters[f];
            	if(tmp[filter.field]){
            		condition = tmp[filter.field];
            	}else{
            		condition = {};
            	};
            	switch (filter.data.type){
    	        	case "date":
    		    		key = "$"+filter.data.comparison;
    		    		if(filter.data.comparison=="eq"){
    		    			condition = filter.data.value ;
    		    		} else {
    		    			condition[key+"e"] = filter.data.value;
//    		    			{
//    	    		    			"$date" : Ext.Date.format(date,'Y-m-d\\TH:i:s.u\\Z')
//    	    		    		};
    		    		}
    		    		//2014-03-11'GMT'08:03:00.000'28800'
    		    	
    	    		break;
    	        	case "treepicker":
    		    		key = "$in";
    		    		value = filter.data.value;
    		    		filter.data.value = value.substring(0, value.length-1);
    		    		condition[key] = filter.data.value.split(",");
            		break;
    	        	case "string":
    	        		key = "$regex";
    	        		condition[key] = filter.data.value;
            		break;
    	        	case "list":
    	        		key = "$in";
    	        		condition[key] = filter.data.value;
            		...
            		break;
    	        	case "boolean":
    	        		key = filter.data.type;
    	        		condition = filter.data.value;
            		break;
    	        }

            	tmp[filter.field] = condition;
        	}
